...
export function withdraw(creep: Creep, structure: AnyStoreStructure, options: {left ?: number, sourcetype ?: ResourceConstant, moveoptions ?: type_movetopos_options} = {}) {
	// 0: move, 1: withdraw
...
export function transfer(creep: Creep, structure: AnyStoreStructure | Creep, options: {sourcetype ?: ResourceConstant, moveoptions ?: type_movetopos_options} = {}) {
	// 0: transfer, 1: move
	if (options.sourcetype == undefined) {
		options.sourcetype = "energy";
	}
...
export function build_structure(creep: Creep, moveoptions: type_movetopos_options = {}): number {
	...
	if (targets.length > 0) {
		let distance = targets.map((e) => creep.pos.getRangeTo(target));
        let argmin = mymath.argmin(distance);
...
export function get_energy(creep: Creep, options: {moveoptions ?: type_movetopos_options, min_energy ?: number, left ?: number} = {}) {
	// 0: found, 1: not found
	if (options.moveoptions == undefined) {
		options.moveoptions = {};
	}
	...
	if (store_structure !== null) {
		withdraw(creep, store_structure, {left: options.left, moveoptions: options.moveoptions});
		return 0;
	}
	return 1;
}
...
export function return_energy_before_die(creep: Creep, moveoptions: type_movetopos_options = {}) {
	...
	if (creep.store.getUsedCapacity("energy") == 0) {
		...
		if (linkcontainer !== null && (<GeneralStore> linkcontainer.store).getFreeCapacity("energy") > 0) {
			transfer(creep, linkcontainer, {moveoptions: moveoptions});
			return 0;
		} else {
			return 1;
		}
...
export function repair_road(creep: Creep, options: {range ?: number, factor ?: number} = {}): number {
...
export function get_structure_from_carry_end(creep: Creep, carry_end: {type: string, name: string}): AnyStoreStructure {
...
export function harvest_with_container(creep: Creep, source: Source, status: {id ?: Id<StructureContainer>, finished ?: boolean, exists ?: boolean} = undefined): number {
	// 0: ok, 1: not adjacent
