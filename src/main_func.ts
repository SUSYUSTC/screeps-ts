...
function update_resources(room_name: string) {
    ...
    for (let external_room_name in pb_status) {
        if (pb_status[external_room_name].status == 0) {
            ...
            mymath.range(5).forEach((e) => pb_attacker_body.push(TOUGH));
            mymath.range(20).forEach((e) => pb_attacker_body.push(ATTACK));
            mymath.range(25).forEach((e) => pb_attacker_body.push(MOVE));
            let pb_healer_body: BodyPartConstant[] = [];
            mymath.range(20).forEach((e) => pb_healer_body.push(HEAL));
            mymath.range(20).forEach((e) => pb_healer_body.push(MOVE));
            ...
            if (pb_status[external_room_name].time_last >= 1000) {
                ...
                mymath.range(40).forEach((e) => pb_carrier_body.push(CARRY));
                mymath.range(10).forEach((e) => pb_carrier_body.push(MOVE));
                let n_carriers = Math.ceil(pb_status[external_room_name].amount / 2000);
                let pb_carrier_name = "pb_carrier" + Game.time.toString();
                let pb_carrier_memory = {
                    "home_room_name": room_name,
                    "role": "pb_carrier",
					"external_room_name": external_room_name,
                }
                mymath.range(n_carriers).forEach((e) => global.spawn_in_queue(room_name, pb_carrier_body, pb_carrier_name + e.toString(), pb_carrier_memory, false));
...
export function set_global_memory() {
	...
	for (let room_name of config.controlled_rooms) {
		...
		let room = Game.rooms[room_name];
		let conf_external = config.conf_rooms[room_name].external_rooms;
		let game_memory = Game.memory[room_name];
		game_memory.external_harvester = {};
		for (var external_room_name in conf_external) {
			game_memory.external_harvester[external_room_name] = {};
			for (var source_name in conf_external[external_room_name].sources) {
    ...
    for (let creepname in Game.creeps) {
        ...
        if (creep.memory.role == 'externalharvester') {
            Game.memory[creep.memory.home_room_name].external_harvester[creep.memory.external_room_name][creep.memory.source_name] = creep.name;
        }
    }
