		...
		if (creep.room.name !== target_room) {
			...
			if (ruin !== undefined) {
				if (creep.withdraw(ruin, "power") == ERR_NOT_IN_RANGE) {
					creep.moveTo(pb_xy[0], pb_xy[1], {range: 1, maxRooms: 1, costCallback: functions.avoid_exits});
					return 0;
				}
			}
			...
			if (creep.pos.getRangeTo(pb_xy[0], pb_xy[1]) > 3) {
				creep.moveTo(pb_xy[0], pb_xy[1], {range: 3, maxRooms: 1, costCallback: functions.avoid_exits});
				return 0;
			}
