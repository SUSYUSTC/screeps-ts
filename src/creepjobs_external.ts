	...
	if (creep.memory.role == 'help_harvester') {
		...
		if (creep.room.name !== creep.memory.external_room_name) {
			...
			if (conf.external_rooms[creep.memory.external_room_name].container) {
				let site = pos.lookFor(LOOK_CONSTRUCTION_SITES).filter((e) => e.structureType == 'container')[0];
		...
		if (creep.room.name == creep.memory.external_room_name && creep.store.getUsedCapacity("energy") > 0 && creep.getActiveBodyparts(WORK) > 0) {
			...
			let road = creep.room.lookForAtArea("structure", ymin, xmin, ymax, xmax, true).map((e) => e.structure).filter((e) => e.structureType == 'road' && e.hits < e.hitsMax * 0.4)[0];
			if (road !== undefined) {
				creep.repair(road);
			}
		}
		...
		if (creep.store.getFreeCapacity("energy") <= 10 || ((creep.room.name !== creep.memory.external_room_name || creep.pos.getRangeTo(pos) > 1) && creep.store.getUsedCapacity("energy") > 0)) {
			if (creep.room.name !== creep.memory.home_room_name) {
				...
				if (conf_external.carry_end.type == 'storage') {
					...
					let link_status = creep.room.memory.named_structures_status.link[conf_external.carry_end.name];
		...
		if (creep.room.name == supporting_room_name) {
			...
			if (creep.store.getUsedCapacity("energy") == 0) {
				...
				if (creep.getActiveBodyparts(WORK) > 0) {
					...
					let ymin = Math.max(creep.pos.y - 1, 0);
					let ymax = Math.min(creep.pos.y + 1, 49);
					let road = creep.room.lookForAtArea("structure", ymin, xmin, ymax, xmax, true).map((e) => e.structure).filter((e) => e.structureType == 'road' && e.hits < e.hitsMax * 0.6)[0];
					if (road !== undefined) {
						creep.repair(road);
					}
				}
				...
				if (store !== undefined) {
					if (creep.transfer(store, "energy") == ERR_NOT_IN_RANGE) {
						basic_job.movetopos(creep, store.pos, 1);
						creep.memory.movable = true;
					}
				}
				return 0;
